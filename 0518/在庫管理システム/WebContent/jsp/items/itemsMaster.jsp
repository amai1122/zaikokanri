...
<body>
	...
	<h1 class="master">商品マスタ一覧</h1>
		...
		<form action="../items/NewItems" method="post">
			...
			<input type="hidden" value="" name="errflag" />
			...
			<tr>
				...
				<td>
					<form action="../items/UpdateItems" name = "${'form' +=itemNo}" method="post">
						...
						<a href="javascript:form<c:out value="${itemNo }" />.submit()" ><c:out value="${item.item_id }" /></a>
					</form>
				</td>
				<!-- 商品名表示 -->
				<td><pre><c:out value="${item.item_name }" /></pre></td>
				<!-- 定価表示 -->
				<td class="price"><pre><c:out value="${item.item_price }" /></pre></td>
				<!-- 原価表示 -->
				<td class="c_price"><pre><c:out value="${item.cost_price }" /></pre></td>
				<!-- 在庫数表示 -->
				<td class="stock"><pre><c:out value="${item.stock_qty }" /></pre></td>
				<td>
				<form action="../stock/NewStock" method="post">
					<input type="hidden" value= <c:out value="${item.item_id }" /> name=item_id >
					<input type="hidden" value= "new" name="flag" />
