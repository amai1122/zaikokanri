...
public class InsertItems extends HttpServlet {
	...
	final static private String ERROR_PATH = "/servlet/items.NewItems";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		ItemsDBAccess check = new ItemsDBAccess();
		String duplicateCheck=check.insertCheckDBAccess(item_id);

        if (item_id.equals("") || item_id == null) {
            errorList.add("「商品ID」を入力してください");
        }else if(item_id.length() != 5) {
        	errorList.add("「商品ID」は5桁で入力してください");
        }else if(( "NG".equals(duplicateCheck)) && ( "new".equals(request.getParameter("flag")))){
        	errorList.add("「商品ID」が重複しています");
        }
