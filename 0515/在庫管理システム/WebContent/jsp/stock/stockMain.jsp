			...
			<tr>
				...
				<td>
					<form action="../stock/UpdateStock" name = "${'form' += stockNo }" method="post">
						...
						<a href="javascript:form<c:out value="${stockNo }" />.submit()" ><c:out value="${stock.getMove_id() }" /></a>
					</form>
				</td>
				<td>

				<c:choose>
					<c:when test="${stock.getReason_id() == '1' }" >1:仕入</c:when>
					<c:when test="${stock.getReason_id() == '2' }" >2:売上</c:when>
					<c:when test="${stock.getReason_id() == '3' }" >3:棚卸</c:when>
					<c:when test="${stock.getReason_id() == '4' }" >4:そのた</c:when>
				</c:choose>
				</td>
				<td><c:out value="${stock.getMove_date() }"  /></td>
				<td><c:out value="${stock.getItem_id() }"  /></td>
				<td><c:out value="${stock.getItem_name() }"  /></td>
				<td>
				<c:choose>
					<c:when test="${stock.getIn_or_out() =='+' }" >＋:入庫</c:when>
