...
public class NewStock extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			// StockBeanオブジェクト生成
	        StockBean stockSession =  (StockBean) session.getAttribute("stock");
	        ...
	        if ("new".equals(flag)) {
	        	...
	        	request.setAttribute("move_id", stockSession.getMove_id());
	        	request.setAttribute("reason_id", stockSession.getReason_id());
	        	request.setAttribute("move_date", stockSession.getMove_date());
	        	request.setAttribute("item_id", stockSession.getItem_id());
	        	request.setAttribute("item_name", stockSession.getItem_name());
	        	request.setAttribute("item_price", stockSession.getItem_price());
	        	request.setAttribute("cost_price", stockSession.getCost_price());
	        	request.setAttribute("stock_qty", stockSession.getStock_qty());
	        	request.setAttribute("in_or_out", stockSession.getIn_or_out());
	        	request.setAttribute("move_qty", stockSession.getMove_qty());
